...
[![crates.io](https://img.shields.io/crates/v/ping-rust.svg)](https://crates.io/crates/ping-rust)
[![GitHub Release](https://img.shields.io/github/v/release/Jyanbai/ping-rust)](https://github.com/Jyanbai/ping-rust/releases/latest)
...
> 当前稳定版与本仓库源码均为 [`v0.1.16`](https://github.com/Jyanbai/ping-rust/releases/tag/v0.1.16)，并已发布至 [crates.io](https://crates.io/crates/ping-rust/0.1.16)。支持 VLESS-Reality-Vision、Hysteria2、TUIC v5、Shadowsocks、AnyTLS、VLESS-TLS-Vision、VLESS-WS-TLS、Trojan-TLS、Trojan-Reality 和 VMess-WS-TLS。用户只选择完整协议，不需要理解或手动组合传输层、安全层与内层协议。
...
Stage 0 自动识别 x86_64/aarch64，从 GitHub Releases 下载对应 musl 静态包，并在首次执行前强制验证 `SHA256SUMS`、归档结构和二进制版本。随后由隐藏的 Rust 安装入口原子写入 `/usr/local/bin/ping-rust`，安全创建 `prs → ping-rust`，并调用现有 Rust `bootstrap` 完成默认 Reality 部署。检测到已有配置时自动跳过，`--no-bootstrap` 可用于只安装管理工具。若系统已有其它 `prs` 命令，安装器会保留它并提示改用 `ping-rust`；升级时只会移除确实指向同目录 `ping-rust` 的旧 `sb` 链接。轻量 Stage 0 协议从 `v0.1.16` 开始；令牌、密码和代理配置都不会被上传。

### 版本兼容

默认命令安装最新稳定版。`v0.1.16` 起，一键安装脚本通过已校验二进制的 `install-self` 完成原子安装。安装 `v0.1.15` 或更早版本时，必须同时使用对应 tag 内的旧脚本：

```bash
bash <(curl --proto '=https' --tlsv1.2 -fsSL \
  https://raw.githubusercontent.com/Jyanbai/ping-rust/v0.1.15/scripts/install.sh) \
  --version v0.1.15
```
